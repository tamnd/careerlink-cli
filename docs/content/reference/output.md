...
describes what users get. Pick a format with `-o`, or let careerlink choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
careerlink <command> -o table   # aligned columns for reading
careerlink <command> -o jsonl   # one JSON object per line, for piping
careerlink <command> -o json    # a single JSON array
careerlink <command> -o csv     # spreadsheet friendly
careerlink <command> -o tsv     # tab-separated
careerlink <command> -o url     # just the URL column
careerlink <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
careerlink <command> --fields id,title,url
```
...
```bash
careerlink <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
careerlink <command>            # a table, because this is a terminal
careerlink <command> | wc -l    # JSONL, because this is a pipe
```
